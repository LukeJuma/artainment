-- Setup Mic Mtaani Platform Tables (Safe Version)
...
DROP POLICY IF EXISTS "mic_mtaani_categories_public_read" ON mic_mtaani_categories;
DROP POLICY IF EXISTS "mic_mtaani_articles_public_read" ON mic_mtaani_articles;
DROP POLICY IF EXISTS "mic_mtaani_events_public_read" ON mic_mtaani_events;
DROP POLICY IF EXISTS "mic_mtaani_businesses_public_read" ON mic_mtaani_businesses;
DROP POLICY IF EXISTS "subscribers_insert_only" ON subscribers;
...
WITH CHECK (true);
-- Insert sample categories (safe with ON CONFLICT)
...
-- Insert sample articles (safe with ON CONFLICT)
...
-- Insert sample events (safe with ON CONFLICT)
...
-- Insert sample businesses (safe with ON CONFLICT)
INSERT INTO mic_mtaani_businesses (name, slug, description, location, phone, category, is_featured) VALUES
('Mama Njeri Kitchen', 'mama-njeris-kitchen', 'Authentic Kenyan cuisine and traditional recipes passed down through generations', 'Kenyatta Avenue, Nakuru', '+254-712-345-678', 'Restaurant', true),
...
-- Success message
SELECT 'Mic Mtaani platform setup completed successfully!' as result;